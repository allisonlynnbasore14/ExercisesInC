...
Yes the final value is 5, for each of the children. Thread diagram is in github repo for this Exercise
...
It appears that the first five are running concurrently because the counter is not changing when the shared counter is printed. When there are more than 5 threads it appears that there is a synchronization error because the additional counters are non-zero.
...
Yes there are duplicates of counter values.
...
Just the counter_array had a time of: 0m0.046s
The mutex counter_array had a time of:0m2.074s

The synchronization caused an extra 2 and 1/2 milliseconds
